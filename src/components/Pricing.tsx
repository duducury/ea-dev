...
export default function Pricing() {
  ...
  return (
    <section id="pricing" className="relative z-10 bg-bg px-6 py-16 md:px-10 md:py-28">
      <div className="mx-auto max-w-7xl" ref={rootRef}>
        ...
        <p className="mt-4 max-w-2xl text-sm text-text-secondary md:text-lg">{t.pricing.note}</p>

        <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-6 md:mt-16 lg:grid-cols-4">
          {t.pricing.plans.map((plan) => (
            <div
              key={plan.name}
              className="pricing-card group flex flex-col rounded-2xl border border-border bg-surface p-6 transition-all duration-300 hover:-translate-y-1.5 hover:border-accent/50 hover:shadow-[0_0_40px_-12px_var(--color-accent-glow)] md:rounded-3xl md:p-8"
            >
